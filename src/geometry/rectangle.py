...
class Rectangle(Shape):
    def __init__(self, size: Vector2, starting_pos: Vector2, colour: Color):
        super().__init__(starting_pos, Vector2(starting_pos.x+(size.x/2),
    ...
    def set_position(self, position: Vector2):
        ...
        self.box_collider.center = Vector2(position.x+(self.rect_like.width/2), position.y+(self.rect_like.height/2))